...
import { Calendar as CalendarIcon, Sparkles, ArrowRight, AlertCircle } from "lucide-react";
...
export default function RentalBookingWidget({
    ...
    const isValidForm = isAvailable && startDate && endDate && days > 0 && !dateValidationError;
    ...
    const handleCreateOrder = async (e: React.FormEvent) => {
        ...
        try {
            const formattedStartDate = new Date(`${startDate}T00:00:00.000Z`).toISOString();
            const formattedEndDate = new Date(`${endDate}T00:00:00.000Z`).toISOString();
            ...
            if (res?.success) {
                ...
                toast.error(res?.message || "Failed to create rental order. Please sign in as a customer.");
            }
        } catch (err) {
            ...
            {days > 0 && !dateValidationError && (
                <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-3 space-y-1.5 text-xs">
                    <div className="flex justify-between text-slate-400">
                        ...
                        <span className="font-semibold text-slate-200">${numPrice.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-slate-400">
