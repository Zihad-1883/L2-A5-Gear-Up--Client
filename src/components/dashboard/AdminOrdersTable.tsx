...
export default function AdminOrdersTable({ initialOrders }: AdminOrdersTableProps) {
    ...
    const filteredOrders = orders.filter((order: TRentalOrder) => {
    ...
    const pendingOrders = orders.filter((o: TRentalOrder) => o.rentalOrderStatus?.toUpperCase() === "PENDING").length;
    const activeOrders = orders.filter((o: TRentalOrder) => ["APPROVED", "PICKED_UP"].includes(o.rentalOrderStatus?.toUpperCase() || "")).length;
    const totalVolume = orders.reduce((sum: number, o: TRentalOrder) => sum + (Number(o.totalPrice) || 0), 0);
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm text-slate-300">
                            ...
                            <tbody className="divide-y divide-slate-800/80">
                                {filteredOrders.map((order: TRentalOrder, idx: number) => {
